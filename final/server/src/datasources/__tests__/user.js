...
describe('[UserAPI.findOrCreateUser]', () => {
  ...
  it('looks up/creates user in store', async () => {
    mockStore.user.findMany.mockReturnValueOnce([{ id: 1 }]);
    ...
    expect(mockStore.user.findMany).toBeCalledWith({
      where: { email: 'a@a.a' },
    });
  });
...
describe('[UserAPI.bookTrip]', () => {
  it('calls store creator and returns result', async () => {
    ...
    mockStore.trip.create.mockReturnValueOnce([{ get: () => 'heya' }]);
    ...
    // make sure store is called properly
    expect(mockStore.trip.create).toBeCalledWith({
      data: { launchId: 1, User: { connect: { id: 1 } } },
    });
  });
});
...
describe('[UserAPI.bookTrips]', () => {
  it('returns multiple lookups from bookTrip', async () => {
    mockStore.trip.create.mockReturnValueOnce([{ get: () => 'heya' }]);
    mockStore.trip.create.mockReturnValueOnce([{ get: () => 'okay' }]);
